...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                ...
                self.LOGGER(__name__).warning("Can't Export Invite link from Force Sub Channel!")
                ...
                self.LOGGER(__name__).info("\nBot Stopped. Enth Mir okke ano entho")
                sys.exit()
        try:
    ...
    async def stop(self, *args):
        await super().stop()
        self.LOGGER(__name__).info("Bot stopped Enth Mir okke ano entho")

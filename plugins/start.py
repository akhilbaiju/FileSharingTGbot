    ...
    if len(text)>7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("Wait Cheyu Please")
        try:
            messages = await get_messages(client, ids)
        except:
            await message.reply_text("Entho Kozhappam patti . wait cheyu!")
            return
...
async def not_joined(client: Client, message: Message):
    text = "<b>You need to join in my Channel/Group to use me\n\nKindly Please join Channel \n Channelil Join Chethal matrame bot use cheyan pattu. Onnu Join Cheythekunney</b>"
